...
- Echo recording mode, enabling users to save the last x seconds of capture
- Lite mode, a lightweight capture option that utilizes the "eye render output" instead of a second camera
- Validation checks for Depth Pre-Pass and Depth Priming
- Disable intervaled rendering settings option, resolving FinalIK culling issues

### Changed

- Default SD quality for recording and streaming now captures at 30fps instead of 60fps

### Fixed

- Forced a 24-bit map with stencil support to resolve depth accuracy issues
- Setup wizard input field responsiveness issues
- Improved file size and streaming throughput without affecting visual quality
- Broken documentation links
- Tablet UI labels visible under certain smoothing settings
- Support disabling registered Audio Capture Marker GameObjects
